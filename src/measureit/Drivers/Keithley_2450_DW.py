...
class Sense2450(InstrumentChannel):
    ...
    def _measure_sweep(self) -> np.ndarray:
        ...
        source.sweep_start()
        buffer_name = self.parent.buffer_name()
        buffer = cast(Buffer2450, self.parent.submodules[f"_buffer_{buffer_name}"])
...
class Source2450(InstrumentChannel):
    ...
    def _set_source(self, setpoint):
        if self._proper_function == "current":
            ...
            raise ValueError(
                "Setpoint must be below gate_ulim: " + str(self.gate_ulim()) + "V"
            )
        elif setpoint < self.gate_llim():
            raise ValueError(
                "Setpoint must be below gate_llim: " + str(self.gate_llim()) + "V"
            )
        else:
            message = f"SOUR:{self._proper_function} " + f"{setpoint}"
            self.write_raw(message)
    ...
    def _set_gate_ulim(self, limit):
        if limit > self.gate_llim():
            ...
            raise ValueError(
                "Upper gate limit must be higher than lower gate limit: "
                + str(self.gate_llim())
            )
    ...
    def _set_gate_llim(self, limit):
        if limit < self.gate_ulim():
            ...
            raise ValueError(
                "Lower gate limit must be lower than upper gate limit: "
                + str(self.gate_ulim())
            )
...
class Keithley2450(VisaInstrument):
    ...
    def __init__(self, name: str, address: str, **kwargs: Any) -> None:
        ...
        self.add_parameter(
            ...
            get_cmd=":SOUR:FUNC?",
            val_mapping={
                key: value["name"] for key, value in Source2450.function_modes.items()
            },
        )
        ...
        self.add_parameter(
            ...
            get_cmd=":SENS:FUNC?",
            val_mapping={
                key: value["name"] for key, value in Sense2450.function_modes.items()
            },
        )
    ...
    def _set_sense_function(self, value: str) -> None:
        ...
        if not isinstance(sense, Sense2450):
            raise RuntimeError(
                f"Expect Sense Module to be of type Sense2450 got {type(sense)}"
            )
    ...
    def _set_source_function(self, value: str) -> None:
        ...
        if not isinstance(source, Source2450):
            raise RuntimeError(
                f"Expect Sense Module to be of type Source2450 got {type(source)}"
            )
