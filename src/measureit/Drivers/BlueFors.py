...
class BlueFors(Instrument):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        channel_magnet: channel of the magnet (optional).
        channel_fse: channel of the FSE (optional).
        """
        ...
        self.add_parameter(
            name="pressure_compressor_outlet",
            unit="mBar",
            get_parser=float,
        ...
        self.add_parameter(
            name="pressure_compressor_inlet",
            unit="mBar",
            get_parser=float,
        ...
        if channel_fse is not None:
            self.add_parameter(
                name="temperature_fse",
                unit="K",
                get_parser=float,
                get_cmd=lambda: self.get_temperature(channel_fse),
                docstring="Temperature of the FSE",
            )

        self.add_parameter(
            name="temperature_still",
            unit="K",
            get_parser=float,
        ...
        self.add_parameter(
            name="temperature_mixing_chamber",
            unit="K",
            get_parser=float,
    ...
    def get_temperature(self, channel: int) -> float:
        ...
        file_path = os.path.join(
            self.folder_path,
            folder_name,
            "CH" + str(channel) + " T " + folder_name + ".log",
        )

        try:
            df = pd.read_csv(
                file_path, delimiter=",", names=["date", "time", "y"], header=None
            )
            ...
            self.log.warning(
                f"Cannot access log file: {err}. Returning np.nan instead of the temperature value."
            )
            return np.nan
        except IndexError as err:
            self.log.warning(
                f"Cannot parse log file: {err}. Returning np.nan instead of the temperature value."
            )
            return np.nan
    ...
    def get_pressure(self, channel: int) -> float:
        ...
        try:
            ...
            self.log.warning(
                f"Cannot access log file: {err}. Returning np.nan instead of the pressure value."
            )
            return np.nan
        except IndexError as err:
            self.log.warning(
                f"Cannot parse log file: {err}. Returning np.nan instead of the pressure value."
            )
            return np.nan
