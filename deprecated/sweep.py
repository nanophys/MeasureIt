        ...
        for i, p in enumerate(self._params):
            ...
            if self.plot is True:
                self.plines[i][self.direction].set_xdata(
                    np.append(self.plines[i][self.direction].get_xdata(), self.setpoint)
                )
...
class Sweep2D:
    def __init__(
        self, inner_sweep_parameters, outer_sweep_parameters, freq, follow_param
    ):
    ...
    def create_figs(self):
        ...
        self.res_out = (
            math.ceil(abs((self.out_stop - self.out_start) / self.out_step)) + 1
        )
    ...
    def run(self):
        ...
        with self.parent.meas.run() as datasaver:
            ...
            while self.parent.running is True:
                ...
                if abs(self.parent.curr_val - self.parent.v_end) <= abs(
                    self.parent.v_step / 2
                ):
