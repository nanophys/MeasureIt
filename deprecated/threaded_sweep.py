...
class BaseSweep(QObject):
    ...
    def start(self, persist_data=None, ramp_to_start=False):
        ...
        elif [str(p) for p in self._params] != [
            key
            for key, val in self.meas.parameters.items()
            if key != "time" and key != str(self.set_param)
        ]:
            self._create_measurement()
...
class Sweep1D(BaseSweep):
    ...
    def ramp_to(self, value, start_on_finish=False, persist=None, multiplier=1):
        ...
        self.ramp_sweep = Sweep1D(
            ...
            complete_func=lambda: self.done_ramping(value, start_on_finish, persist),
            save_data=False,
            plot_data=True,
        )
        self.is_running = False
...
class Sweep2D(BaseSweep):
    ...
    def start(self):
        ...
        """
        print(
            f"Starting the 2D Sweep. Ramping {self.set_param.label} to {self.out_stop} {self.set_param.unit}, while sweeping {self.in_param.label} between {self.in_start} {self.in_param.unit} and {self.in_stop} {self.in_param.unit}"
        )
    ...
    def update_values(self):
        ...
        if abs(self.out_setpoint - self.out_stop) >= abs(self.out_step / 2):
            ...
            time.sleep(self.outer_delay)
            print(
                f"Setting {self.set_param.label} to {self.out_setpoint} {self.set_param.unit}"
            )
            self.set_param.set(self.out_setpoint)
            time.sleep(self.outer_delay)
            # Reset our plots
...
class PlotterThread(QThread):
    ...
    def __init__(
        self, sweep, plot_bin=1, setaxline=None, setax=None, axesline=None, axes=[]
    ):
    ...
    def add_break(self, direction):
        ...
        for i, p in enumerate(self.sweep._params):
            self.axesline[i][direction].set_xdata(
                np.append(self.axesline[i][direction].get_xdata(), np.nan)
            )
            self.axesline[i][direction].set_ydata(
                np.append(self.axesline[i][direction].get_ydata(), np.nan)
            )
    ...
    def create_figs(self):
        ...
        if self.sweep.set_param is not None:
            ...
            self.setax.set_ylabel(
                f"{self.sweep.set_param.label} ({self.sweep.set_param.unit})"
            )
        ...
        for i, p in enumerate(self.sweep._params):
            ...
            self.axes.append(
                self.fig.add_subplot(self.grid[4 * row : 4 * (row + 1) - 1, col])
            )
        ...
        plt.subplots_adjust(
            left=0.2, right=0.9, bottom=0.1, top=0.9, wspace=0.4, hspace=0.4
        )
    ...
    def update_plots(self, force=False):
        ...
        updates = 0
        if (
            len(self.data_queue) >= self.plot_bin or force is True
        ) and self.figs_set == True:
            while len(self.data_queue) > 0:
                ...
                if self.sweep.set_param is not None:
                    ...
                    self.setaxline.set_xdata(
                        np.append(self.setaxline.get_xdata(), time_data[1])
                    )
                    self.setaxline.set_ydata(
                        np.append(self.setaxline.get_ydata(), set_param_data[1])
                    )
                    self.setax.relim()
                ...
                for i, data_pair in enumerate(data):
                    self.axesline[i][direction].set_xdata(
                        np.append(self.axesline[i][direction].get_xdata(), x_data)
                    )
                    self.axesline[i][direction].set_ydata(
...
class HeatmapThread(QThread):
    ...
    def create_figs(self):
        ...
        cbar.set_label(
            f"{self.sweep.in_sweep._params[1].label} ({self.sweep.in_sweep._params[1].unit})"
        )
    ...
    def set_database(self):
        ...
        try:
            initialise_or_create_database_at(
                "C:\\Users\\erunb\\measureit.Databases\\" + db + ".db"
            )
