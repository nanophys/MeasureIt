...
def test_sweep0d_metadata_written(qtbot, temp_measureit_home):
    ...
    s = Sweep0D(max_time=0.05, inter_delay=0.01, save_data=True, plot_data=False, suppress_output=True)
    ...
    assert 'measureit' in meta_map, "Expected 'measureit' metadata to be present for Sweep0D"
    meta = json.loads(meta_map['measureit']) if isinstance(meta_map['measureit'], str) else meta_map['measureit']
...
def test_sweep1d_metadata_written(qtbot, temp_measureit_home):
    ...
    s = Sweep1D(p.x, start=0.0, stop=0.1, step=0.1, inter_delay=0.01, save_data=True, plot_data=False, suppress_output=True)
    s.follow_param(p.parabola)
    ...
    ds = _load_ds(captured)
    meta_map = getattr(ds, 'metadata', {}) or {}
    assert 'measureit' in meta_map, "Expected 'measureit' metadata to be present for Sweep1D"
    meta = json.loads(meta_map['measureit']) if isinstance(meta_map['measureit'], str) else meta_map['measureit']
...
def test_sweep2d_metadata_prefers_outer_and_writes_once(qtbot, temp_measureit_home):
    ...
    s = Sweep2D(in_params, out_params, outer_delay=0.0, inter_delay=0.01, save_data=True, plot_data=False, suppress_output=True)
    ...
    ds = _load_ds(captured)
    meta_map = getattr(ds, 'metadata', {}) or {}
    assert 'measureit' in meta_map, "Expected 'measureit' metadata to be present for Sweep2D"
    meta = json.loads(meta_map['measureit']) if isinstance(meta_map['measureit'], str) else meta_map['measureit']
    assert meta.get('class') == 'Sweep2D', f"Expected class 'Sweep2D' in metadata, got {meta.get('class')}"
